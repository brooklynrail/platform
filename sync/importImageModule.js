...
async function importImageModule(data, issue_folder, client) {
  try {
    if (data && data.path) {
      // Function to check if a image exists in Directus
      const checkImageExists = async (path, client) => {
      ...
      const existingImageId = await checkImageExists(data.path, client);
      if (existingImageId) {
        console.log("Image already exists", existingImageId);
